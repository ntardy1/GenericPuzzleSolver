...
class Puzzle {
    ...
    // 2D array representing the state of the board
    std::array<std::array<int, BOARD_COLUMN_NUM>, BOARD_ROW_NUM> m_board;
    // 1D vector of eligible candidates for an empty board position
    std::vector<int> m_candidates;
    // `std::pair` holding the validity of the board (`bool`) and the number of
    // solved elements (`int`)
    ...
    /// @return `std::pair` of a `bool` representing the validity of the board
    // (`true` for valid; `false` otherwise) and an `int` representing the number
    // of filled elements
    ...
    /// @return The index location of `target` in `vector`; `vector.end()` if `target`
    // is not in `vector`
